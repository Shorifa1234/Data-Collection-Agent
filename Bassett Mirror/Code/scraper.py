...
def scrape_product(detail_url: str) -> list[dict]:
    ...
    # Bassett Mirror renders specs as plain text between "SPECIFICATIONS" and "DESCRIPTION".
    ...
    if spec_match:
        ...
        lines = [l.strip() for l in spec_block.split("\n") if l.strip()]
        i = 0
        while i < len(lines):
            line = lines[i]
            if ":" in line:
                ...
                value = parts[1].strip()
                # If value empty, next line is the value
    ...
    # ── 5. Description ───────────────────────────────────────────────────
    # The DESCRIPTION section follows specs with bullet-point style lines.
    ...
    if desc_match:
        ...
        desc_lines = [
            l.strip() for l in desc_raw.split("\n")
            if l.strip() and len(l.strip()) > 8
        ]
        if desc_lines:
            row["Description"] = " | ".join(desc_lines[:10])
